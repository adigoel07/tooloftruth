...
- **Dashboard (LIVE)**: zero-dep local server (`localhost:4321`, auto-started by daemon) serving a single-file dashboard — Overview (verdict gauge + needle), Alerts (filter + copy-to-investigate blocks), Sessions, Conversations, Costs, Git scan, Settings. Light/dark theme with system detection + persistence. Verified rendering 105 receipts live.
